-------------------------------------------------------------------------------
-- Spec of the pumas.SphericalPoint metatype
...
describe('SphericalPoint', function ()
    describe('constructor', function ()
        ...
        it('should properly set keyword attributes', function ()
            local t = pumas.UnitaryTransformation()
            local c = pumas.SphericalPoint{r = 1, theta = 2, phi = 3, frame = t}
            assert.is.equal(c.r, 1)
            assert.is.equal(c.theta, 2)
            assert.is.equal(c.phi, 3)
            assert.is.equal(c.frame, t)
        ...
        it('should initialise from another instance', function ()
            local t = pumas.UnitaryTransformation()
            local c0 = pumas.SphericalPoint(1, 2, 3, t)
            local c1 = pumas.SphericalPoint(c0)
            assert.is.equal(c1.r, 1)
    ...
    describe('get', function ()
        ...
        it('should return an equivalent C array', function ()
            local c = pumas.SphericalPoint(3, 0, 0)
            local a = c:get()
        ...
        it('should transform back to the simulation frame', function ()
            ...
            local c = pumas.SphericalPoint(3, 0, 0, t)
            local a = c:get()
            assert.is.equal(a[0], 0)
            assert.is.equal(a[1], 0)
            assert.is.equal(a[2], -3)
    ...
    describe('set', function ()
        ...
        it('should accept a C array', function ()
            local c = pumas.SphericalPoint()
            c:set(ffi.new('double [3]', 0, 0, 3))
            assert.is.equal(c.r, 3)
        ...
        it('should accept another point instance of same type', function ()
            local c = pumas.SphericalPoint()
            local t = util.Reflection()
            c:set(pumas.SphericalPoint(1, 2, 3, t))
            assert.is.equal(c.r, 1)
            assert.is.equal(c.theta, 2)
            assert.is.equal(c.phi, 3)
            assert.is.equal(c.frame, t)
        ...
        it('should accept another point instance of different type', function ()
            local c = pumas.SphericalPoint()
            local t = util.Reflection()
            c:set(pumas.CartesianPoint(0, 0, 3, t))
            assert.is.equal(c.r, 3)
            assert.is.equal(c.theta, 0)
            assert.is.equal(c.phi, 0)
            assert.is.equal(c.frame, t)
    ...
    describe('transform', function ()
        ...
        it('should do nothing when same frame', function ()
            local c = pumas.SphericalPoint(1, 2, 3)
            c:transform()
            assert.is.equal(c.r, 1)
            assert.is.equal(c.theta, 2)
            assert.is.equal(c.phi, 3)
        ...
        it('should transform between two frames', function ()
            ...
            local c = pumas.SphericalPoint(3, 0, 0, t0)
