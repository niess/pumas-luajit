...
describe('LocalFrame', function ()
    describe('constructor', function ()
        ...
        it('should propely set the transform', function ()
            ...
            assert.is.equal(util.round(f.matrix[0][0], 7), 0)
            assert.is.equal(util.round(f.matrix[1][0], 7), 1)
            assert.is.equal(util.round(f.matrix[2][0], 7), 0)
            assert.is.equal(util.round(f.matrix[0][1], 7), -s2)
            assert.is.equal(util.round(f.matrix[1][1], 7), 0)
            assert.is.equal(util.round(f.matrix[2][1], 7), s2)
            assert.is.equal(util.round(f.matrix[0][2], 7), s2)
            assert.is.equal(util.round(f.matrix[1][2], 7), 0)
            assert.is.equal(util.round(f.matrix[2][2], 7), s2)
        end)
    end)
end)
